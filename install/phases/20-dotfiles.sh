#!/usr/bin/env bash
#
# omagub phase: dotfiles — clone your dotfiles and lay them over $HOME.
...
if [ ! -d "$REPO/.git" ]; then
  ...
  git clone "$OMAGUB_DOTFILES_REPO" "$REPO"
else
...
ssh_url="$(echo "$OMAGUB_DOTFILES_REPO" | sed -E 's#^https?://github.com/#git@github.com:#')"
...
for entry in "$REPO"/.*; do
  ...
  skip=0
  for ex in "${OMAGUB_DOTFILES_EXCLUDE[@]}"; do [ "$base" = "$ex" ] && skip=1; done
